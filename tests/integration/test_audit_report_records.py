...
async def test_native_summary_rereads_previously_verified_records(
    native_audit, mutation
):
    ...
    async def during(kwargs):
        ...
        if kwargs["output_format"]["schema"] != WRITE_BACK_SCHEMA or changed:
            return
        records = [
            row for row in board.comments if row.body.startswith("[native-audit:")
        ]
        if len(records) < 2:
            return
        first = records[0]
