...
async def test_native_lane_discovery_keeps_failure_kind(
    native_audit, monkeypatch, failure_kind
):
    audit, executor, backend, _tracker, _git, workspace, _repository = native_audit
    original = backend.call_tool
    error = (
        RuntimeError("independent adapter implementation defect")
        if failure_kind == "programmer"
    ...
    async def call(*, name, arguments):
        ...
        if name == "list_comments":
            ...
            if failure_kind == "outage":
                backend._tool_errors["list_comments"] = (
                    "independent unavailable service"
                )
            else:
                raise error
    ...
    if failure_kind == "outage":
        ...
        assert any(
            "independent unavailable" in row.reason
            for row in caught.value.report.scopes[0].unavailable
        )
    else:
