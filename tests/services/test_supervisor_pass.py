...
@pytest.mark.parametrize(
    "stopped", ["the lane observation", "the scope read", "the scope arm"]
)
async def test_cancellation_is_not_swallowed(stopped):
    """No boundary contains the caller's stop, only a lane's or a scope's failure.

    The tick draws three: one around a lane's observation, one around a
    scope's read and one around a scope's stage-barrier observation. A stop
    is not the failure of the lane or the scope it arrived in, so it comes
    straight back out of the tick — nothing is written down as having failed
    and no scope is counted as unobserved.
    """
    port = await board(lanes=LANES)
    ...
    if stopped == "the scope read":
        ...
        tally = supervisor(port)
    elif stopped == "the scope arm":
        readings = {REF: ready_set()}
        tally = supervisor(port)
        scope_arm = {REF: asyncio.CancelledError()}
    else:
        readings = {REF: ready_set()}
        tally = Cancelling(
            tracker=port,
    ...
    with structlog.testing.capture_logs() as logs:
        with pytest.raises(asyncio.CancelledError):
            await pass_over(
                port, readings=readings, tally=tally, scope_arm=scope_arm
            ).run(FIXTURE_EPOCH)
    ...
    assert [
        ...
        if entry["event"]
        in {
            "supervisor_scope_failed",
            "supervisor_lane_failed",
            "supervisor_scope_arm_failed",
        }
    ] == []
