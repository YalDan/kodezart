...
fails as loudly as a new bypass: the state moves KOD-806 holds outside
this seam, and the writes that still reach the port
...
async def test_an_in_run_author_reaching_for_the_graph_writes_nothing_and_reports_it(
    ...
    for write in journal.writes:
        if write.method in artifact_writes():
