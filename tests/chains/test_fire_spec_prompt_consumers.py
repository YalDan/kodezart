...
CORPUS = list(tickets())
#: The consumer sites ``capture_prompts`` returns, in capture order; the
#: single source both the capture and the base key set are built from.
CAPTURE_SITES = ("implementation", "fix", "workflow_pr")
...
async def capture_prompts(family, ticket, monkeypatch):
    ...
    return dict(
        zip(
            CAPTURE_SITES,
            (
                quality_gate.calls[0]["prompt"],
                fix_runner.calls[0]["prompt"],
...
#: The authored base this suite refuses to let the recorded file restate: one
#: digest over the canonical JSON of the recorded file's base entries, spelled
#: out here rather than read back from the file, so a re-baseline of an entry
#: fails even when the file and the consumers agree with each other.  A site or
#: a case that did not exist yet may be added to the file; one covered here may
#: not move.
BASE_DIGEST = "9c0e30bc3660959fe9888bb852c75e8da2dfd0ee258a4ff9a728f3228a7ab379"
...
def base_digest(recorded):
    base = {key: recorded.get(key) for key in base_keys()}
...
def test_recorded_goldens_are_extended_and_never_rebaselined():
    ...
    recorded = {
        ...
        for family, cases in golden["prompts"].items()
        for index, sites in cases.items()
        for site, digest in sites.items()
    }
    assert base_digest(recorded) == BASE_DIGEST
    extended = recorded | {
        f"{V5_SET}/{len(CORPUS)}/{CAPTURE_SITES[0]}": "0" * 64,
        f"{V5_SET}/0/branch_name": "1" * 64,
    }
    assert base_digest(extended) == BASE_DIGEST
