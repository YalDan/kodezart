...
async def test_failed_preparation_releases_uncheckpointed_unwritten_workspace(
    ...
    async def unavailable_capture(workspace_path, *, repository_path):
        identity = await actual_identity(
            workspace_path, repository_path=repository_path
        )
    ...
    try:
        with pytest.raises(
            (NativeWriteRefusalError, FireSpecEntryError, GitOperationError)
        ):
        ...
        assert not workspace._workspaces, (
            "failed preparation retains uncheckpointed ownership"
        )
