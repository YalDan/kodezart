"""A claim session must observe the selected commit's original bytes."""
...
from tests.tracker.test_audit_claim import REQUEST
from tests.tracker.test_audit_claim import server as server
from tests.tracker.test_audit_claim import setup as setup


@pytest.mark.parametrize("phase", ["clean", "before", "during"])
@pytest.mark.parametrize("namespace", ["default", "configured"])
async def test_actual_claim_reuses_cache_without_accepting_replaced_bytes(
    ...
    verifier = build()
    verifier._cache = native.cache
    verifier._git = native.git
    verifier._workspace = native.workspace
    verifier._remote = "origin"
    request = REQUEST.model_copy(
        update={"repo_url": native.source.as_uri(), "cache_key": "audit-cache"}
    )
    acquired = []
    ...
    if phase == "clean":
        ...
        assert result.head_sha == native.head
...
async def test_unreadable_replacement_refs_cannot_return_claim(
...
@pytest.mark.parametrize("read_number", [1, 2])
async def test_replacement_read_settles_before_claim_workspace_release(
