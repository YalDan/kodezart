"""KOD-89-AC-5 — the fan-out cap is configuration, prefixed and bounded.

Every numeric constant in this project is an ``AppConfig`` field with the
``KODEZART_`` prefix, so the cap is asserted the same way: it reads from the
environment, it refuses values outside its declared range at construction,
and the range itself is the one the fire-time ruling recorded.
"""
