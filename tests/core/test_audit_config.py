...
def test_audit_defaults_and_retired_fields():
    config = AppConfig()
    assert config.audit_sweep_interval_seconds is None
...
def test_bounds_load_through_environment(monkeypatch, value):
    ...
    monkeypatch.setenv("KODEZART_AUDIT__TIMEOUT_SECONDS", "30")
    config = AppConfig()
...
def test_full_interval_cannot_be_shorter_than_tick():
    with pytest.raises(ValidationError, match="must not be shorter"):
        AppConfig(
            ...
            audit_sweep_interval_seconds=120,
            audit_full_sweep_interval_seconds=60,
        )
