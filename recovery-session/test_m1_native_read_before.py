...
async def test_actual_configured_constructor_reads_current_native_criterion():
    operation = OperationConfig(operation_name='fixture', workspace='fixture', issue_labels={'criterion':'Configured Check'})
    server = FakeLinearMcpServer(issues=[FakeMcpIssue(id='parent'),FakeMcpIssue(id='child',parent_id='parent',labels=['Configured Check'])])
    tracker,_ = build_tracker(backend=TrackerBackend.LINEAR,retry=RetryPolicy(attempts=1,initial_delay=0),operation=operation,caller=server)
