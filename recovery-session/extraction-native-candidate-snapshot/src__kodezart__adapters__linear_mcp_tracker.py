...
from kodezart.types.domain.surface import DescriptionWriteAuthority, SurfaceKind, SurfaceLease, WritableSurface
...
class LinearMcpTracker:
    ...
    async def edit_description(
        self, *, target: str, expected: str, replacement: str, authorization: DescriptionWriteAuthority | None = None
        ...
        if authorization is not None:
            ...
            async def attempt() -> DescriptionEditResult:
                ...
                if surface.kind is SurfaceKind.CRITERION_SUB_ISSUE:
                    require_criterion_source(expected=original.model_copy(update={"body": current.body}), current=current)
                elif original.model_dump(exclude={"body", "updated_at"}) != current.model_dump(exclude={"body", "updated_at"}):
                    raise TrackerProtocolError("description target facts changed", tool=_TOOL_GET_ISSUE, detail=target)
