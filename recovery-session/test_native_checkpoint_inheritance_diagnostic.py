...
async def test_actual_parent_saved_resume_after_native_external_effect(repository, monkeypatch, position):
    ...
    try:
        try:
            ...
            print("NATIVE_RESUME_EXCEPTION", position, type(exc).__name__, str(exc))
        print("NATIVE_RESUME_DISPATCH", position, [call["output_format"]["schema"]["title"] for call in second_executor.calls])
        after = latest_checkpoints(saver)
        print("NATIVE_RESUME_ACTUAL", json.dumps({"position": position, "checkpoints": after}, sort_keys=True))
        print("NATIVE_RESUME_ARCHIVES", len([c for c in port.comments if c.body.startswith("[fixture-amendment:")]))
    finally:
