...
async def test_requested_root_cannot_be_replaced_by_another_native_issue():
    server = ScopeMcpServer()
    server.issue_detail_updates[ROOT.key] = {"id": "foreign-root"}
    tracker = linear_over_fake_mcp(server)
    with pytest.raises(ScopeReadError):
        await tracker.scope_issues(ref=ROOT)
...
class _RefusingBoundary(ScopeMcpServer):
    ...
    async def call_tool(self, *, name: str, arguments: Mapping[str, object]) -> McpToolResult:
        self.attempts += 1
        raise self.failure


@pytest.mark.parametrize("credential", [False, True])
async def test_native_failure_preserves_exact_cause_and_never_repeats_unsafe_write(credential):
    if credential:
        failure = McpCredentialRefusedError("authority refused", server_name="scope-test", tool_name="save_issue")
        expected = TrackerAccessDeniedError
    else:
        failure = McpCallUnansweredError("write receipt absent", server_name="scope-test", tool_name="save_issue")
