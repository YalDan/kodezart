...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("{{#each issue_body}}{{this}}{{/each}}", "requires a list"),
    ],
)
async def test_invalid_shape_is_a_real_dispatch_failure(body, message):
    owner, board, executor, _ = build(
        changes={PromptKey.GROOMING_PASS: {"rubric_body": body}}
    )
