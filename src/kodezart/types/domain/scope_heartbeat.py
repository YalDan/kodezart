...
class HeartbeatOutcome(StrEnum):
    ...
    Five members, and they partition: the scope's run was started here, a
    run of it is already live, its last run here ended with every lane done
    and its members have not moved since, approval is not on it yet, or
    asking cost an exception. ``UNAPPROVED`` and ``FAILED`` are never one
    member — a scope nobody has approved is the ordinary resting state of a
    declared row, and a read that raised is a fault.
    """
...
class HeartbeatEntry(CamelCaseModel):
    ...
    submitted, the live one that stopped a submission, or — for
    ``CONVERGED`` — the job whose converged ending was latched. ``detail``
    is the failure's own rendering and nothing else, so a FAILED line says
    which read refused rather than that something did.
    """
