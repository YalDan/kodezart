...
Every observer the supervisor tick runs keeps its whole memory in the one
record at its ``(subject, signal)`` address. The writing is the same for all
of them: a lease on exactly that address, the record rewritten under it, and
the transition its lane's stream still owes posted after it. It is kept in
one place so that an observer states only what the address should hold.
...
class RunAlarmRecorder:
    ...
    async def read(
        ...
        return await self._tracker.read_run_alarm(
            issue_key=issue_key, subject=subject, signal=signal
        )
