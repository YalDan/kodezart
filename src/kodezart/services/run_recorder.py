...
Both producers — the pass scheduler and the lifecycle watcher —
...
read exactly like a refused page.  A sink failing outside the
...
so the recorder VERIFIES the destination for THIS
...
duplicate of it.  The title rides both outcome events, because
...
async def report_record_failure(
    ...
    *,
    ...
    empty.  A destination that refused is named with what it
...
class RunRecorder:
    ...
    async def record(self, record: RunRecord) -> RunRecordResult:
        ...
        "verified", for a row nothing had written.
        """
