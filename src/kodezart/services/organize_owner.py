        ...
        *,
        ...
        for phase in self._phases:
            ...
            for _convergence_round in range(self._policy.max_convergence_rounds):
                ...
                # own, and it is released before any halt is written. A
                # member whose own approval reading is not this row's side
                # of approval is left out: a run may hold it, and this row
                ...
                try:
                    async with RunSurfaceLease(
                        ...
                        for issue in subjects:
                            ...
                            for _admission_round in range(
                                ...
                                route = await self._route(
                                    result,
                                    issue=issue,
                                    scope_issue_keys=frozenset(members),
                                )
                                ...
                                findings = tuple(
                                    f for f in findings if f not in repaired
                                )
                                # Parent edits may remove this subject; splits may
                                # add newly minted members. Continue on the actual
                                # membership.
                                ...
                                if (
                                    ...
                                    and (
                                        ...
                                        or any(
                                            c.state_kind
                                            is not WorkflowStateKind.CANCELED
                                            for c in children
                                        )
                                    )
                                ):
                        ...
                        if (
                            ...
                            and all(
                                [
                                    await self._admission.is_live(result)
                                    for result in fresh
                                ]
                            )
                        ):
                            ...
                            current_labels = await self._tracker.read_scope_labels(
                                ref=scope
                            )
                            marker_subjects = [
                                revision.issue
