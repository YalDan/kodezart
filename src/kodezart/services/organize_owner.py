...
class OrganizeOwner:
    ...
    async def _converge(
        ...
        *,
        ...
        The bound is the configured one and it is read here, once. A first
        round that admits nobody, or that finds no subject, leaves the phase
        idle; every later round spends its dry round. A round that settles
        writes the phase marker onto every admitted subject and stops. Every
        other exit is a report the caller returns at once, including the
        exhaustion of the bound.
        """
                ...
                async with RunSurfaceLease(
                    ...
                    for issue in subjects:
                        ...
                        for _admission_round in range(
                            ...
                            # entry above asked, of the children the session
                            # left: an item the session left out keeps the
                            # stage owed.
                            children = await self._tracker.read_criteria(
                                issue_key=issue.issue_key
                            )
