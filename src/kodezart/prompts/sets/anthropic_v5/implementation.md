...
A second worker kind runs on this machine on its own quota: Codex. Dispatch it like any lane, with `codex exec --dangerously-bypass-approvals-and-sandbox --skip-git-repo-check -C UNIT_WORKTREE -m gpt-6-astra -c model_reasoning_effort=ultra "TASK"`: one unit per call, the branch, base and pull request it delivers to, and the evidence you expect in its final message; count it toward the load hold. A Codex lane never writes to the tracker and never opens, merges or closes a pull request; you record its results from its report.
