...
{{/each}}
Read the issues, the initiatives they serve, and the repositories they name; then
correct what the board asserts about them, and where the board puts them.
...
- issues sitting outside that tree, re-placed through the fields that carry placement;
...
{{records.grooming.name}}, the {{records.grooming.system}} destination {{records.grooming.id}}, titled EXACTLY
...
ran.{{/if}}{{#if records.grooming_absent}}No record destination
