...
{{/if}}{{#if phase_ticket}}The ticket rubric. A member's body is complete when a builder who reads only that body can build it without inventing a decision: what changes, where, and how it is shown working are all stated. Give the body the Check / Do / Evidence shape, a Check that can be shown true or false, a Do that says the work, and an Evidence row left empty for the graded commit. Rewrite the body where it falls short; leave what already holds.
