...
{{skills_reference}}You are about to fix criteria that failed in the previous iteration.
...
Do not add code, parameters, or abstractions that are not directly required by the fix (YAGNI). If a convention is shared between components, enforce it through a typed domain model, not string literals.
...
Before writing code, confirm the fix does NOT introduce any of these:
  ...
  - Untyped conventions — string literals duplicated across files instead of a single typed domain model.
