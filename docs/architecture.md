...
| EscalationResolutionReader | LinearMcpTracker | Whether a decision record answers one escalation; narrowed out of the port rather than added to it, and holding no write |
| EscalationAgeingReader | LinearMcpTracker | The reads an escalation's age is measured from — its occurrence and lane record comments and its resolution — narrowed out of the port, holding no write |
...
a record issue. A held lane's questions are aged, and its tally is not observed:
a held lane is waiting on a person, and the ageing alarm is the alarm for that.
A tally raise standing on it stays as it stood until the lane is ready again.
...
`services.tally_supervisor.TallySupervisor` is the first observer the tick
runs, and `services.run_alarm_recorder.RunAlarmRecorder` is the one leased
writer both observers write through.
...
second observer (KOD-892). Once per scope it reads every member's run-state
record the ready read names — ready, blocked, unapproved, closed and held — as
the scope's position; a damaged record leaves that scope's questions unobserved
for the tick, logged as `supervisor_escalations_unobserved`, while its
tallies are still observed. Per ready lane and per held lane it asks, for each
criterion of the lane's roster, whether the lapse question a lapsed observation raises
(KOD-699) is on the lane's issue, through `EscalationRecordReader.find`. For
each one there it reads the question's record at
`(EscalationSubject, ESCALATION_AGEING)`, feeds the recorded question, the
lane's commits and the tick-age count from the stored anchor to
`services.escalation_signals.observe_recorded_escalation_ageing`, and writes
through the recorder only when whether the question is raised changes. Its
transition events are keyed per question, `escalation_ageing:<occurrence>`,
so two questions on one lane are two streams; the tally arm's key stays
`tally_unmoved`. Only lapse questions are aged.
...
through the same leased recorder as the tally arm; other escalation
occurrences are not aged. These readers do not declare the complete signal
table or a boot capability for it.
...
lane tally and escalation ageing arms write through it. These bounded record reads do not provide
...
rule before any write. Before it posts, it reads the container's recent status
updates through `ScopeStatusReader` and posts only when its report differs from
the latest report of this operation already there; a read that refuses
propagates before any write, and a person's note on the same surface is passed
...
tally and escalation ageing arms only.
...
leased writer that exists observes the lane tally and escalation ageing arms
only.
