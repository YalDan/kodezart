...
reads, so the scope walker sees a lane close with no further write. The
audit's terminal precondition consults that one arithmetic over the direct
criterion family the port returns, and nothing writes a parent's finished
state.
