...
| `KODEZART_FIRE_PREP_PASS_GATE_SIGNALS` | `list[PassSignal]` | `["issues_changed", "triage_backlog", "reviews_changed"]` |  | Signals the fire-preparation pass is gated on. Three, because its prompt gathers three streams and a gate covering fewer would skip real work: the standing triage backlog it re-sweeps whole, issue activity since the last tick, and review threads — which are a separate object class no issue scan reaches. Dropping `triage_backlog` is the usual edit on a board that parks plan stubs at triage, since that signal is true while any exist. |
