...
| `KODEZART_PREP_PASS_INTERVAL_SECONDS` | `float` | `900.0` | >= 10.0, <= 86400.0 | Seconds between fire-preparation passes. A separate knob from the dispatch interval because the two passes cost differently: dispatch is a free port call on a quiet board, preparation spends a session on every window that is not empty. |
| `KODEZART_GROOMING_PASS_INTERVAL_SECONDS` | `float` | `3600.0` | >= 10.0, <= 86400.0 | Seconds between grooming passes. Each one runs a repository's whole declared check chain, so the interval is the cost. |
| `KODEZART_PASS_SESSION_TIMEOUT_SECONDS` | `float` | `1800.0` | >= 1.0 | Wall-clock bound on one judgment-pass session. Exceeding it is TIMEOUT, and a pass that timed out writes nothing at all. |
| `KODEZART_PASS_SESSION_WORKING_DIR` | `str` | `/tmp/kodezart-pass-session` |  | Working directory a preparation session runs in. It composes from a work set already rendered into its prompt and reaches nothing, so this is deliberately not a repository checkout. |
| `KODEZART_GROOMING_PASS_ALLOWED_TOOLS` | `list[str]` | `["Bash", "Glob", "Grep", "Read"]` |  | Tools a grooming session may use inside the repository checkout it verifies. Non-empty by design: a verification session that cannot run the repository's own commands would be reporting a build it did not perform. |
