...
| `KODEZART_TRACKER__SERVER_NAME` | `str` | `linear` |  | Identity of the vendor MCP server the tracker adapter dials. One consumer: the transport factory building the programmatic client on the deterministic path, which stamps this name on every transport log line and error. |
...
nested environment names such as `KODEZART_TRACKER__SERVER_NAME`. The earlier flat tracker
