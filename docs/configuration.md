...
| `KODEZART_DISPATCH_WORKFLOW` | `str` | `fire` | `fire`, `scope` | Which workflow the scheduled dispatcher submits runs to. `fire` builds the v0.2 dispatch pass per declared repository, with its lifecycle watcher and claim heartbeat, and no scope heartbeat. `scope` builds no dispatch pass and schedules the scope heartbeat over the declared `[[organize_scopes]]` rows, so no scheduled run starts without a scope. Grooming, fire prep and the organize tick run under either. |
...
| `KODEZART_TRACKER__SURFACE_LEASE_SECONDS` | `float` | `900.0` | >= 60.0, <= 86400.0 | Bound for write-surface leases held by a writing run's job id. Renewal is explicit; no background task extends these leases. |
